...
function countVowels(str) {
  ...
  for (let i = 0; i < arrayStr.length; i += 1) {
    if (arrayStr[i].match(`[aeiouyAEIOUY]`)) {
      counter += 1;
    }
  }
  return counter;
...
function isPalindrome(str) {
  const strCheck = str.replaceAll(/[!?,\s]/g, '').toLowerCase();
  return Array.from(strCheck).reverse().join('') === strCheck;
}
...
function reverseWords(str) {
  ...
  return array.map((word) => Array.from(word).reverse().join('')).join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => code[code.indexOf(letter) + 13]);
}
